...
-- Image empty -> shows the original library's triangle cursor, colored by Library.AccentColor.
...
local function UpdateMouseIcon()
    ...
    elseif Drawing then
        ...
        MouseTriangle.Color = Library.AccentColor;
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input, Processed)
        	if (not Picking) and not Processed then
        		if KeyPicker.Mode == 'Toggle' and ParentObj.Value and KeyPicker then
        			local Key = KeyPicker.Value;
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleLayout = Library:Create('UIListLayout', {
            Padding = UDim.new(0, 4);
            FillDirection = Enum.FillDirection.Horizontal;
            HorizontalAlignment = Enum.HorizontalAlignment.Right;
            SortOrder = Enum.SortOrder.LayoutOrder;
        ...
        -- Dynamically resize ToggleRegion so it never overlaps addons.
        local function UpdateToggleRegion()
            local RegionPosition = ToggleRegion.AbsolutePosition.X
            local FirstAddonX = nil
    
            for _, Child in ipairs(ToggleLabel:GetChildren()) do
                if Child:IsA('GuiObject') and Child ~= ToggleLayout then
                    local ChildX = Child.AbsolutePosition.X
    
                    if FirstAddonX == nil or ChildX < FirstAddonX then
                        FirstAddonX = ChildX
                    end
                end
            end
            ...
            if FirstAddonX then
                local NewWidth = FirstAddonX - RegionPosition - 4
    
                ToggleRegion.Size = UDim2.fromOffset(
                    math.max(0, NewWidth),
                    ToggleOuter.AbsoluteSize.Y
                )
            else
        ...
        local function ScheduleToggleRegionUpdate()
            task.defer(UpdateToggleRegion)
        end
    
        ToggleLabel.ChildAdded:Connect(ScheduleToggleRegionUpdate)
        ToggleLabel.ChildRemoved:Connect(ScheduleToggleRegionUpdate)
    
        ToggleLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(
            ScheduleToggleRegionUpdate
        )
        ...
        ToggleLabel:GetPropertyChangedSignal('AbsoluteSize'):Connect(
            ScheduleToggleRegionUpdate
        )
        ...
        function Toggle:SetValue(Bool)
            ...
            for _, Addon in next, Toggle.Addons do
                if Addon.Type == 'KeyPicker' and Addon.SyncToggleState then
                    Addon.Toggled = Bool
                    Addon:Update()
                elseif Addon.Type == 'KeyPicker' and not Bool and Addon.Toggled then
                    Addon.Toggled = false
                end
            end
    ...
    function Funcs:AddRainbowAccentToggle(Idx, Info)
        ...
        Toggle:OnChanged(function(Value)
            ...
            if Value then
                -- Snapshot whatever accent color is active right now, before the
                -- rainbow loop starts overwriting Library.AccentColor every frame.
                -- This works regardless of how the accent was set (ThemeManager,
                -- a manual Library.AccentColor = ... at startup, or the default),
                -- since we don't depend on Options.AccentColor existing at all.
                Library.PreRainbowAccentColor = Library.AccentColor;
            else
                -- Prefer the AccentColor color picker's value if one exists
                -- (keeps it in sync if the user changed it via ThemeManager
                -- while the rainbow was off), otherwise fall back to the
                -- snapshot taken when the rainbow was turned on.
                local Restored = (Options.AccentColor and Options.AccentColor.Value)
                    or Library.PreRainbowAccentColor
    ...
    function Funcs:AddMouseIconChanger(Idx, Info)
        ...
        -- Toggle (Toggles[Idx]) followed by a textbox (Options[Idx .. 'Image']) that takes a
        -- Roblox image id / rbxassetid. With the textbox empty, the default triangle cursor
        -- is used and it follows the library's accent color.
    ...
    function Funcs:AddDropdown(Idx, Info)
     ...
     function Dropdown:SetValues(NewValues)
        	...
        	if Dropdown.SpecialType ~= 'Player' then
        		if Dropdown.Multi then
        			for key in pairs(Dropdown.Value) do
        				if not table.find(Dropdown.Values, key) then
        ...
        function Dropdown:SetValue(Val)
            if Dropdown.Multi then
                ...
                if (not Val) or Val == nil then
                    for Value, Bool in next, Dropdown.Values do
                        nTable[Value] = false
                    end;
        ...
        Dropdown:BuildDropdownList();
             if Dropdown.Multi then
                local allFalse = true
                if typeof(nTable) == "table" then
                    for Value, Bool in next, nTable do
                        if Bool == true then
                            allFalse = false
                            return
                        end
...
local function OnPlayerChange()
	...
	for _, Value in next, Options do
		if Value.Type == 'Dropdown' and Value.SpecialType == 'Player' then
			...
			if Value.Multi then
				for selectedPlayer in pairs(Value.Value) do
					if not table.find(PlayerList, selectedPlayer) then
						Value.Value[selectedPlayer] = nil
						changed = true
					end
				end
			else
				if Value.Value and not table.find(PlayerList, Value.Value) then
					Value.Value = nil
					changed = true
				end
			end
			...
			if changed then
				Library:SafeCallback(Value.Callback, Value.Value)
				Library:SafeCallback(Value.Changed, Value.Value)
				Library:AttemptSave()
			end
		end
	end
